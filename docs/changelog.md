...
## 2026-08-18 - v0.1.0 首发（核心启动器）

### 更新概述

首个正式发布版本，聚焦**核心启动器**能力：账号、下载、加载器、实例、启动、主题、国际化与无障碍。社区服务器、服主后台等**云端模块**将在后续版本开放。

### 特性

1. **账号管理** — 微软 / 离线 / 第三方账户，Token 存系统密钥环（`#[serde(skip)]` 保证不落盘）
2. **下载管理** — 游戏版本下载（分块并发 + SHA1 校验 + 镜像限流重试）、Fabric/Forge/NeoForge 加载器
3. **实例管理** — PCL2 式目录结构（`game/layout.rs` = 路径唯一事实源），全局资源共享
4. **启动** — 版本化目录启动、日志采集、运行状态
5. **主题系统** — 暗/亮/终端预设 + 强调色 + 预览
6. **无障碍** — 光敏模式（纯黑 + 禁用动画）、高对比度、字体缩放至 200%、高 DPI 适配
7. **灵动岛导航 + 双角色系统**（服主侧云端能力敬请期待）

### 变更与清理

- 版本号统一为语义化 `semver`（Cargo 与 Tauri 配置强制合法 semver，MSI/NSIS 需纯数字正式版）
- `bump-version.js` 修复 import 副作用；新增 `version:release` 命令（清除预发布后缀）
- README / tauri bundle 元数据 / Cargo license（GPL-3.0-only）补齐
- 新增 GitHub Actions 三平台 CI 与 draft Release
- 补充单元测试：Rust（账户 UUID、下载限流/取消）、前端（字体缩放、主题 DOM、无障碍 class）
