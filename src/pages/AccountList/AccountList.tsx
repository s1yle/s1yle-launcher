    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] p-4 md:p-8">
      <div className="max-w-6xl w-full space-y-8">
            ...
            <div className="bg-surface rounded-lg p-4 border border-border-hover">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <h3 className="text-xl font-bold text-text-primary">{currentAccount.name}</h3>
                    <p className="text-text-secondary">{getAccountTypeText(currentAccount.account_type)}</p>
                    <p className="text-sm text-text-tertiary">最后登录: {formatTime(currentAccount.last_login_time)}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm text-text-tertiary">创建时间: {formatTime(currentAccount.create_time)}</p>
                  <p className="text-sm text-text-tertiary">UUID: {currentAccount.uuid.substring(0, 8)}...</p>
                </div>
              </div>
            </div>
          ) : (
            <div className="bg-surface rounded-lg p-6 border border-border-hover text-center">
              <p className="text-text-secondary text-lg">当前没有选中任何账户</p>
              <p className="text-text-tertiary">请从下方选择一个账户</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-surface backdrop-blur-sm rounded-xl p-6 border border-border-hover">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-text-primary">账户列表</h2>
            <button 
              onClick={handleAddAccount}
              className="px-4 py-2 bg-info hover:bg-info text-text-primary rounded-lg transition-colors flex items-center gap-2"
            >
              <span>➕</span>
              添加账户
            </button>
          ...
          {isLoadingAccounts ? (
            ...
            <div className="text-center py-8">
              <p className="text-text-secondary text-lg">暂无账户</p>
              <p className="text-text-tertiary">点击上方按钮添加第一个账户</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {accounts.map((account) => (
                <div 
                  key={account.uuid}
                  className={`bg-surface rounded-lg p-4 border ${currentAccount?.uuid === account.uuid ? 'border-info' : 'border-border'} hover:border-border-hover transition-colors`}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center">
                      <div className="w-10 h-10 rounded-full bg-info/30 flex items-center justify-center">
                      ...
                      <div className="ml-3">
                        ...
                        <p className="text-sm text-text-secondary">{getAccountTypeText(account.account_type)}</p>
                      </div>
                    </div>
                    {currentAccount?.uuid === account.uuid && (
                      <span className="px-2 py-1 bg-info text-text-primary text-xs rounded-full">当前</span>
                    )}
                  </div>
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-text-secondary">{formatTime(account.create_time)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-text-tertiary">最后登录</span>
                      <span className="text-text-secondary">{formatTime(account.last_login_time)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-text-tertiary">UUID</span>
                      <span className="text-text-secondary font-mono text-xs">{account.uuid.substring(0, 8)}...</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex gap-2">
                    {currentAccount?.uuid !== account.uuid && (
                      <button
                        ...
                        className="flex-1 py-2 bg-success hover:bg-success text-text-primary rounded-lg transition-colors text-sm"
                      >
                        设为当前
                      </button>
                    )}
                    <button
                      ...
                      className="flex-1 py-2 bg-error hover:bg-error text-text-primary rounded-lg transition-colors text-sm"
                    >
                      删除
                    </button>
                  </div>
        ...
        footer={
          <div className="flex justify-end gap-4">
            <button 
              ...
              className="px-4 py-2 bg-surface hover:bg-gray-700 text-text-primary rounded-lg transition-colors"
              disabled={loading}
            >
              取消
            </button>
            <button 
              ...
              className="px-4 py-2 bg-info hover:bg-info text-text-primary rounded-lg transition-colors disabled:opacity-50"
              disabled={loading}
            >
              {loading ? "添加中..." : "确认添加"}
            </button>
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="block text-text-primary mb-2">账户名称</label>
            <input 
              type="text" 
              placeholder="请输入账户名称 (1-16字符)"
              className="w-full px-4 py-2 bg-surface border border-border-hover rounded-lg text-text-primary placeholder-gray-400 focus:outline-none focus:border-info"
              value={accountName}
              onChange={(e) => setAccountName(e.target.value)}
              disabled={loading}
            />
            <p className="text-text-tertiary text-sm mt-1">支持字母、数字和下划线</p>
          </div>
          <div>
            <label className="block text-text-primary mb-2">账户类型</label>
            <select 
              className="w-full px-4 py-2 bg-surface border border-border-hover rounded-lg text-text-primary focus:outline-none focus:border-info"
              value={accountType}
          ...
          footer={
            <div className="flex justify-end gap-4">
              ...
              <button 
                ...
                className="px-4 py-2 bg-error hover:bg-error text-text-primary rounded-lg transition-colors"
              >
                确认删除
              </button>
            </div>
