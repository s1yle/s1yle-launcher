  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        const global = await getGlobalGameSettings();
        setSettings(global);
        lastSavedSettings.current = JSON.stringify(global);
        isInitialLoad.current = true;
        // 开关状态与持久化配置同步（无 min/max 即自动分配中）
        setAutoMemory(!global.min_memory && !global.max_memory);

        // 扫描常见的 Java 安装路径
        const commonJavaPaths = await scanJavaInstallations().catch(() => []);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        console.error('[GlobalGameSettings] memory refresh failed:', e);
      }
    };
    refresh();
