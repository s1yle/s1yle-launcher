...
/// 添加新账户（offline / thirdparty）
/// 微软账户必须通过 poll_and_complete_login（后端全程 OAuth，token 绝不经过前端）
#[command]
pub fn add_player_account(name: String, account_type: String) -> Result<String, String> {
    if name.is_empty() {
...
#[command]
pub fn save_login_state(login_state: StoreLoginState) -> Result<(), String> {
    set_login_state_internal(login_state)
}

/// 清除登录状态（登出时调用）
#[command]
pub fn clear_login_state() -> Result<(), String> {
    set_login_state_internal(StoreLoginState::default())
}
...
pub fn delete_account(app: tauri::AppHandle, uuid: String) -> Result<String, String> {
    ...
    crate::account::manager::save_accounts_to_disk_internal()?; // 修改后自动保存
    auto_select_default_account()?;
