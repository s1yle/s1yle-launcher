...
pub async fn get_xbox_live_validation(
    access_token: &String,
) -> Result<XboxLiveAuthResponse, String> {
    ...
    if !status.is_success() {
        ...
        return Err(format!("Xbox Live 认证失败: {}", body));
    }

    let auth_response = resp.json::<XboxLiveAuthResponse>().await.map_err(|e| {
        println!("[get_xbox_live_validation] 解析响应失败: {}", e);
        format!("解析 Xbox Live 响应失败: {}", e)
    })?;
...
pub async fn get_xsts_validation(
    ...
    if !status.is_success() {
        ...
        return Err(format!("XSTS 认证失败: {}", body));
    }

    let auth_response = resp.json::<XboxLiveAuthResponse>().await.map_err(|e| {
        println!("[get_xsts_validation] 解析响应失败: {}", e);
        format!("解析 XSTS 响应失败: {}", e)
    })?;
...
pub async fn get_minecraft_access_token(
    ...
    if !response_text.contains("access_token") {
        ...
        return Err(format!("Minecraft login failed: {}", response_text));
    }

    let login_response: MinecraftLoginResponse = serde_json::from_str(&response_text).map_err(|e| {
